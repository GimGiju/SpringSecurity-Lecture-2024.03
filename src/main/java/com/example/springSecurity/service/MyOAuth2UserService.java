...
public class MyOAuth2UserService extends DefaultOAuth2UserService {
    ...
    // Provider(구글,깃허브 등 *네이버, 카카오는 안됨) 로부터 받은 userRequest 데이터에 대해 후처리하는 메소드
    @Override
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        switch (provider){
            ...
            case "github":
                ...
                uid = provider + "_" + id;
                securityUser = securityService.getUserByUid(uid);
                if (securityUser == null){              // 가입이 안되어져 있으면 가입 진행 하는 방식 / 가입 되어있으면 빠져나감.
                    uname = oAuth2User.getAttribute("name");
                    ...
                    securityService.insertSecurityUser(securityUser);
                    securityUser = securityService.getUserByUid(uid);
                    log.info("깃 허브 계정을 통해 회원가입이 되었습니다.");
                }
                break;
            case "naver":
            ...
            case  "kakao":
                ...
                if (securityUser == null){              // 가입이 안되어져 있으면 가입 진행 하는 방식 / 가입 되어있으면 빠져나감.
                    ...
                    email = (email == null) ? "kakaoTempEmail@naver.com" : email;
